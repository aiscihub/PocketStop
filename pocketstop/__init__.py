"""PocketStop v0.1 replay path (NEXT_ACTIONS.md sections 3-4).
...
PocketStop is a standalone repository. It is developed against, and its
tests depend on, the sibling `valleyfevermutation` repository's
`ai2sci_p4l3` pipeline -- specifically the `compare` package and its
already-computed `results/p4_l3_100ns_v1/` outputs, which are this
...
to this repo's root). Set POCKETSTOP_AI2SCI_P4L3 to override the location
...
_REPO_ROOT = _HERE.parent.parent  # this repo root (parent of the pocketstop/ package dir)
AI2SCI_P4L3 = Path(os.environ.get(
    "POCKETSTOP_AI2SCI_P4L3",
